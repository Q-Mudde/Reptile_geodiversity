### R scrip Q Masters project DRIVE VERSION


# installing and opening packages  (some of these are only used for practice)
...
# all there is to do now is add the geodiveristy data to this dataframe and modelling can start
...
######################
### testing/ coding trash########
####################
plot(salvagrid) 
plot(tdwg)
plot(tdwg[tdwg$HBWID == bb, ])
test1 <- tdwg[tdwg$HBWID %in% bb,] #### this works
plot(test1, add = F, col = "red")
dev.off()
aa <- cbind(salvagrid$HBWID)
bb<- as.vector(aa)
dih_col <- which(dih_y2$MemeberID %in% memb)
test1 <- which(tdwg %in% bb)
test2 <- tdwg[tdwg$HBWID %in% aa, ]
test3 <- tdwg[tdwg$HBWID %in% salvagrid$HBWID, ] #this works and is most basic
identical(test1,test3)


# this seems to work here with the allturtles data
# next, it is needed to create a full data.frame with all HBWID cells
# this complete data.frame must include spcount and all geodiveristy index values per HBWID cell
# so we must transform the Versteegh (2016) geodiversity index rasterdata into the HBWID data.frame
# thus the biggest challenge, working with the geodiveristy raster

rawgeo = raster(file.path('rawdata/raw_geo/GDI10.tif'))
rawgeo
geocrs <- st_crs(rawgeo)
rawgeo
tdwg
plot(tdwg)
...
pol2 <- rasterToPolygons(r, fun=function(x){x>2})
plot(r)
r
pol
plot(pol)
...
polys1 <- rasterToPolygons(rawgeo, fun = function(x){x>0})
list1 <- over(polys1, tdwg, returnList = T)
names(list1) <- 1:length(list1)
testdf <- ldply(list1, .id = "gridcell")
crs(rawgeo)
crs(tdwg)
plot(world$geom)
colom <- world[world$name_long == "Colombia",]
plot(world$geom)
plot(colom$geom, add = T, col = "red")
crs(world)
crs(colom) <- geocrs
type(world)
plot(tdwg)
crop1 <- crop(rawgeo,colom)
crop1
colom
crs(tdwg)
plot(crop1)
...
r2
r
proj4string(r2) <- geocrs
r2
geodiv100T <- raster(file.path('rawdata/raw_geo/Geodiv100Tot.tif'))
geodiv10T <- raster(file.path('rawdata/raw_geo/Geodiv10Tot.tif'))
geofix <- raster(file.path('rawdata/raw_geo/GDI10km.tif'))
geofix
tdwg
rawgeo
geotest78 <- raster(file.choose())
geotest78
geotest91 <- raster(file.choose())
geotest91

######################################################################
########## THIS IS WHERE THE CODING TRASH IS POSTED FOR NOW###########
######################################################################
a <- funQ(x= "vermiculatus.shp", dir = 'subsets', tdwg = tdwg, type = "polygon")
??rasterToPolygons
b <- funoverlap(x = "vermiculatus.shp", dir = 'subsets', tdwg = tdwg, type = "polygon")

###   testing quickCO function
test <- quickCO(a)
test
test2 <- quickCO(c)
identical(test2, C2)
...
A1 <- a[,c("HBWID", "SpecName")]
A1
A1$SpecName <- 1
names(A1)[2] <- "Spcount"
A1
aggregate(.~ HBWID, data = A1, FUN = sum)
C1 <- c[,c("HBWID", "SpecName")]
C1$SpecName <- 1
names(C1)[2] <- 'SpCount'
C2 <- aggregate(.~HBWID, data = C1, FUN = sum)
hist(C2$SpCount)
#######
...
#letstry the function
overlapTDWG('vermiculatus.shp', tdwg=tdwg, dir = 'subsets', type = "polygon" )
#to overlap rasters with tdwg polygon they need to have the same extend, resolution& crs
rawgeo
tdwg
crs1 <- crs(tdwg)
rawgeo1 <- rawgeo
...
crs(rawgeo1)
rawgeo1
rawgeo2 <- aggregate(rawgeo1, 10, fun = mean)
??aggregate


overlapTDWG(x = "Lacerta agilis", dir = getwd(), tdwg = tdwg, polygon = rawdata, type = 'polygon' )
#nope, nu knippen en plakken
overlapList <- over(lacerta1, tdwg, returnList = TRUE)
names(overlapList) <- 1:length(overlapList)
overlapDF <- ldply(overlapList, .id = "polygon")
# trouble with sf an spdf
tdwg
lacerta
lacerta1 <- as(lacerta, "Spatial")
names(rawdata)
rawdata$binomial <- rawdata$Binomial
# NOT RUN {
r <- raster(nrow=18, ncol=36)
r[] <- runif(ncell(r)) * 10
r[r>8] <- NA
...
RangeToTDWGGrid <- function(x, ...) {
  rastertemp = raster(extent(tdwg), resolution = 1, crs = 4326)
  rasterval = rasterize(x, rastertemp, field = 1, fun = 'count')
  rasterplot = plot(rasterval, ...)
  rasterplot = plot(world$geom, add= T)
  print(rasterplot)
  return(rasterval)
}
RangeToGrid(lacerta, res = 0.5, map = T)
RangeToTDWGGrid(lacerta)
dev.off()
RangeToGrid(turtledata, res = 2, map = T)
RangeToTDWGGrid(turtledata)
turtleraster <- RangeToTDWGGrid(turtledata)
overlapTDWG(x = 'turtleraster.tif',dir = getwd(), tdwg = tdwg1)
RangeToTDWGGrid()
lizraster <- RangeToTDWGGrid(lizdata)
writeRaster(turtleraster, 'turtleraster.tif', overwrite = T)
tdwg1 <- as(tdwg, "Spatial")
overlapTDWG(x = turtleraster,tdwg = tdwg1)
st_crs(tdwg)
crs(tdwg1) <- "+proj=longlat +datum=WGS84 +no_defs"
st_crs(turtledata)
crs(turtleraster) <- "+proj=longlat +datum=WGS84 +no_defs"
turtleraster
...
rawgeo1 = rawgeo
crs(rawgeo1) = 4326
crs(rawgeo1) = "+proj=longlat +datum=WGS84 +no_defs"
rawgeo1
st_crs(turtle_ras)
st_crs(rawgeo1)
turtle_ras
#### plotting rawgeo crashes laptop
#### aggregating rawgeo to decrease compt time didnt work
## fun reptile maps
lizdata = rawdata[rawdata$Group == 'lizard', ]

rast2 = raster(extent(lizdata), resolution = 0.5,
               crs = st_crs(lizdata)$proj4string)
liz_ras = rasterize(lizdata, rast2, field = 1, fun = 'count')
plot(liz_ras)
head(lizdata)
anole_data = lizdata[lizdata$Genus == "Anolis",]
plot(anole_data$geometry)
horns = lizdata[lizdata$Binomial ==  "Anolis proboscis",]
plot(horns$geometry, add = T, col = "blue", cex = 4)
library(stringr)
rast3 = raster(extent(anole_data), resolution = 1,
               crs = st_crs(anole_data)$proj4string)
anole_ras = rasterize(anole_data, rast3, field = 1, fun = 'count')
plot(anole_ras)
...
RangeToGrid(anole_data, res = 1)
snake_data = rawdata[rawdata$Group == "snake",]
RangeToGrid(snake_data, res = 5)
RangeToGrid(anole_data, res = 3)
?function()
  RangeToGrid(anole_data, res = 0.3)
natrix_data = snake_data[snake_data$Genus == 'Natrix', ]
head(natrix_data)
RangeToGrid(natrix_data, res=0.5, col= heat.colors(3), map = T)
bothrops_data = snake_data[snake_data$Genus == 'Bothrops',]
RangeToGrid(bothrops_data, res = 1)
plot(world$geom, add = T)
RangeToGrid(snake_data, res = 2)
plot(world$geom)
RangeToGrid(anole_data, res=0.5, map = T, col = heat.colors(10))
st_crs(anole_data)
st_crs(world)
st_crs(anole_data) <- 4326
RangeToGrid(snake_data, res = 1, map = T)
RangeToGrid(anole_data, res = 0.5, map= T)
coralsnakes = snake_data[snake_data$Genus== c('Leptomicrurus', 'Micruroides', 'Micrurus'), ]
coralsnakes
tail(coralsnakes)
RangeToGrid(coralsnakes, res = 0.5, map = T)
RangeToGrid(anole_data, res = 0.5, map=T)
phelsuma = lizdata[lizdata$Genus == 'Phelsuma', ]
RangeToGrid(phelsuma, res = 0.5, map = T, col = heat.colors(3))
RangeToGrid(turtledata, res = 1, map = T)
trapelus = lizdata[lizdata$Genus == "Trapelus", ]
RangeToGrid(trapelus, res = 0.5, map = T)
acanthodactylus = lizdata[lizdata$Genus == 'Acanthodactylus',]
RangeToGrid(acanthodactylus, res = 0.5, map = T, col = heat.colors(5))
eremias = lizdata[lizdata$Genus == "Eremias", ]
RangeToGrid(eremias, res = 0.5, map = T, col = heat.colors(6))
natrix_data
coralsnakes
snake_data
RangeToGrid(coralsnakes, res = 0.5, map = T, col= rainbow(5))
coral = coralsnakes[coralsnakes$Binomial == "Micrurus fulvius", ]
coralsnakes
coralsnakes["Binomial",]
coral
RangeToGrid(coral, res = .5, map = T)
RangeToGrid(turtledata, res = 1, map = T, col = rainbow(10))
RangeToGrid(lizdata, res = 1, map = T, col = rainbow(10))
cham = lizdata[lizdata$Genus == 'Chamaeleo',]
RangeToGrid(cham, res = 0.4, map = T, col = rainbow(4))
RangeToGrid(snake_data, res = 1, map = T, col = rainbow(10))
RangeToGrid(turtledata, res = 1, map = T, col = rainbow(10))
RangeToGrid(rawdata, res = 1, map = T, col = rainbow(10))
lacerta <- rawdata[rawdata$Binomial == 'Lacerta agilis', ]
RangeToGrid(lacerta, res = 1, map = T)
